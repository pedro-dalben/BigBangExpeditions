...
| **T20 — Boundary seam** | BLOCKED | **OBSERVED** | sector reset with persistent neighbors shows small per-cycle deltas concentrated in boundary-straddling content (cycle-003 deltas; lootr ±1, furnace/campfire swaps). Visual seam inspection still pending (no client) — counts prove the effect exists |
